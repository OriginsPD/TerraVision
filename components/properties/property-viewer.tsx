...
import { Box, Image as ImageIcon, Maximize2, Move3D, Play, Loader2, ZoomIn, Info, MousePointer2, Smartphone, Minimize2, Sparkles } from "lucide-react"
...
function Model({ url }: { url: string }) {
  ...
  // Ensure all materials look good
      ...
      <div className={cn(
        ...
        <div className="absolute top-6 left-6 right-6 z-30 flex items-center justify-between pointer-events-none">
          <div className="flex items-center gap-3 pointer-events-auto">
            <div className="glass px-1 py-1 rounded-2xl flex items-center gap-1">
              <Button 
                variant={activeTab === "3d" ? "glossy" : "ghost"} 
                size="sm"
                onClick={() => setActiveTab("3d")}
                className="rounded-xl px-4"
                disabled={!property.has3D}
              >
                <Box className="h-4 w-4 mr-2" />
                3D Experience
              </Button>
              <Button 
                variant={activeTab === "photos" ? "glossy" : "ghost"} 
                size="sm"
            ...
            {property.has3D && activeTab === "3d" && (
              <Badge className="bg-primary/20 text-primary border-primary/30 backdrop-blur-md px-4 py-2 rounded-xl flex gap-2">
                ...
                <span className="font-bold uppercase tracking-widest text-[10px]">AI Immersive Model</span>
              </Badge>
            )}
          </div>
          ...
          <div className="flex items-center gap-3 pointer-events-auto">
            <button 
              ...
              className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 border border-white/20 text-white backdrop-blur-md hover:bg-white/20 transition-all shadow-xl"
            >
        ...
        {activeTab === "3d" && property.has3D ? (
          <div className="h-full w-full relative group/canvas">
            <Suspense fallback={
              <div className="flex h-full w-full flex-col items-center justify-center gap-6 bg-gradient-to-br from-background via-card to-background">
                ...
                <div className="text-center">
                  <p className="text-xl font-bold text-foreground">Loading Immersive Scene</p>
                  <p className="text-sm text-muted-foreground">Preparing high-fidelity 3D assets...</p>
                </div>
              </div>
            }>
            ...
            {/* Walkthrough Controls Hint Overlay */}
            <AnimatePresence>
                  ...
                  <div className="glass p-10 rounded-[3rem] text-center border-4 border-primary/30 max-w-lg pointer-events-auto">
                    <div className="flex justify-center mb-6">
                      <div className="h-20 w-20 rounded-3xl bg-primary/20 flex items-center justify-center">
                        <Move3D className="h-10 w-10 text-primary" />
                      </div>
                    </div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="flex flex-col gap-1">
                          <div className="flex gap-1">
                            <kbd className="px-2 py-1 rounded bg-white/20 border border-white/30 text-xs">W</kbd>
                            <kbd className="px-2 py-1 rounded bg-white/20 border border-white/30 text-xs">A</kbd>
                            <kbd className="px-2 py-1 rounded bg-white/20 border border-white/30 text-xs">S</kbd>
                            <kbd className="px-2 py-1 rounded bg-white/20 border border-white/30 text-xs">D</kbd>
                          </div>
                          <span className="text-xs font-bold text-muted-foreground uppercase">Movement</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <MousePointer2 className="h-6 w-6 text-primary" />
                        <div className="flex flex-col">
                          <span className="font-bold">Click to Look</span>
                          <span className="text-xs text-muted-foreground">Capture mouse for 360 view</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <Smartphone className="h-6 w-6 text-accent" />
                        <div className="flex flex-col">
                          <span className="font-bold">Virtual Joystick</span>
                          <span className="text-xs text-muted-foreground">Available on touch devices</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="px-2 py-1 rounded bg-white/20 border border-white/30 text-xs font-bold">ESC</div>
                        <div className="flex flex-col">
                          <span className="font-bold">Exit Mode</span>
                          <span className="text-xs text-muted-foreground">Release mouse or exit</span>
                        </div>
                      </div>
                    </div>
                    <Button variant="glossy" className="w-full py-7 text-lg rounded-2xl" onClick={() => setShowControlsHint(false)}>
            ...
            {isWalkthrough && (
              <div ref={joystickRef} className="absolute bottom-10 left-10 h-48 w-48 touch-none z-30" />
            )}

            {/* Bottom Controls Bar */}
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-30 flex items-center gap-4 p-2 glass rounded-[2.5rem] border border-white/20 shadow-2xl transition-all hover:scale-105">
              {isWalkthrough ? (
                <div className="flex items-center gap-2 pl-4">
                  <span className="text-sm font-bold text-white/80 pr-4 border-r border-white/10">Active Walkthrough</span>
                  <Button variant="destructive" size="sm" className="rounded-full px-6 py-5 font-bold" onClick={() => setIsWalkthrough(false)}>
                    Exit Navigation
                  </Button>
                </div>
              ) : (
                <>
                  <div className="flex items-center gap-6 px-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-white/70 uppercase">Orbit</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="h-8 w-8 rounded-full bg-white/10 flex items-center justify-center">
                        <ZoomIn className="h-4 w-4 text-accent" />
                      </div>
                      <span className="text-xs font-bold text-white/70 uppercase">Zoom</span>
                    </div>
                  </div>
             ...
             <div className="grid h-full grid-cols-4 grid-rows-2 gap-3 p-4">
                ...
                {property.images.slice(1, 3).map((img, i) => (
                  <div key={i} className="overflow-hidden rounded-[1.5rem] relative group/sub">
                    <img 
                      src={img.startsWith('http') || img.startsWith('/') ? img : `http://localhost:8000${img}`} 
                ...
                {property.images.length > 3 ? (
                  <div className="relative overflow-hidden rounded-[1.5rem] bg-card/50 backdrop-blur-md border border-white/20 flex flex-col items-center justify-center group/more cursor-pointer">
                    <img 
                      src={property.images[3].startsWith('http') || property.images[3].startsWith('/') ? property.images[3] : `http://localhost:8000${property.images[3]}`} 
                      className="absolute inset-0 h-full w-full object-cover opacity-20 blur-sm group-hover/more:scale-110 transition-transform duration-700"
                      alt="More photos"
                    />
                   ...
                   <div className="rounded-[1.5rem] bg-white/5 border border-white/10 border-dashed flex items-center justify-center">
        ...
        <div className="flex items-center gap-3 overflow-x-auto pb-4 pt-2 no-scrollbar scroll-smooth">
          {property.has3D && (
            <button
              onClick={() => setActiveTab("3d")}
              className={cn(
                "flex h-20 w-28 shrink-0 flex-col items-center justify-center rounded-2xl border transition-all duration-300 relative overflow-hidden group",
                activeTab === "3d" ? "border-primary bg-primary/20 ring-4 ring-primary/20 shadow-lg" : "border-white/10 bg-white/5 hover:bg-white/10"
              )}
            >
          ...
          {property.images.map((img, i) => (
            <button
              ...
              className={cn(
                ...
                activeTab === "photos" ? "border-primary ring-4 ring-primary/20 shadow-lg" : "border-white/10 hover:border-white/30"
              )}
            >
              <img 
                src={img.startsWith('http') || img.startsWith('/') ? img : `http://localhost:8000${img}`} 
                className={cn("h-full w-full object-cover transition-opacity duration-300", activeTab === "photos" ? "opacity-100" : "opacity-40 group-hover:opacity-100")}
                alt={`Thumbnail ${i + 1}`}
              />
